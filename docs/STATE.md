...
- **PENDÊNCIA ABERTA — extração LLM two-pass:** `ingest-text`/`add_to_memory` (5.1) e `ingest` PDF NUNCA rodados ponta-a-ponta com LLM real — ambiente sem `ANTHROPIC_API_KEY`. Testes cobrem só roteamento c/ fake ingester. Validar quando houver key.
